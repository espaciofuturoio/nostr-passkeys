...
const encodeNsec = (privateKeyHex: string): string => {
    const privWords = bech32.toWords(Buffer.from(privateKeyHex, 'hex'))
    return bech32.encode('nsec', privWords)
}

// Generate a mnemonic : maybe later we can use it to generate a wallet
export const generateMnemonic = async (): Promise<string> => {
    // // Convert the mnemonic to a seed
    // const seed = await bip39.mnemonicToSeed(mnemonic)
    // // Derive a private key from the seed
    // const privKey = secp.utils.hashToPrivateKey(seed.slice(0, 32))
...
export const useNostr = () => {
    ...
    const getPrivateKey = async () => {
        ...
        try {
            const privKey = await retrievePrivateKey()
            const nsec = encodeNsec(privKey) // Encode the private key
            setNsec(nsec)
